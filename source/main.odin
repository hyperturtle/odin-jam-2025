...
init :: proc() {
	...
	for i in 0 ..< 50 {
		...
		append(&storms, Storm{(vec2{rand.float32(), rand.float32()} - 0.5) * 1E4, {0, 0}, {}, r})
	}
}
...
update :: proc() {
	...
	rl.BeginMode2D(camera)
	{
		for storm in storms {
			...
			rl.DrawLineV(storm.pos, storm.pos + storm.vel, rl.RED)
			rl.DrawLineV(storm.pos, storm.pos + storm.acc, rl.BLUE)
		}
