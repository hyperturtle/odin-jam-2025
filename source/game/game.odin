...
find_mst :: proc(
	...
	for &node, i in nodes {
		if node.enabled || (ghost && node.ghost) || (ghost_node != nil && ghost_node == &node) {
...
init :: proc() {
	...
	gen_circle :: proc($T: int, $div: int, mod: int = 0) -> [T]vec2 {
		circle: [T]vec2
		for i in 0 ..< T * div {
			if i % div != mod {
				continue
			...
			circle[i / div] = vec2{math.sin(angle), math.cos(angle)}
		}
		return circle
	}
	...
	for outer, x in gen_circle(6, 1) {
		for inner, y in gen_circle(7, 1) {
			if abs(x + y) % 4 == 0 {continue}
			node: Node
